...
- форма-конструктор **«Предложить идею»** с автоматическим скачиванием `.txt`-заявки;
...
- аккуратные микроанимации с поддержкой `prefers-reduced-motion`;
- клавиатурная навигация, skip-link и улучшенные focus-состояния;
- SEO: canonical, robots, sitemap и Schema.org JSON-LD;
- Open Graph для превью ссылок во VK с отдельной обложкой 1200×630;
- оптимизированная загрузка шрифтов WOFF2 и hero-изображений;
...
В релизной сборке настроены:

- уникальные `title` и `description` для публичных страниц;
- `canonical`, Open Graph для VK и общая social-preview обложка;
- Schema.org JSON-LD для организации, сайта, страниц и хлебных крошек;
- `robots.txt` и `sitemap.xml`;
- skip-link, `:focus-visible`, управление мобильным меню с клавиатуры и `aria-live` для результата формы;
- отключение необязательной анимации при `prefers-reduced-motion`;
- WOFF2-версии основных шрифтов и отложенная загрузка следующих кадров hero-слайдера.
